...
% FileLocationMesh = strcat(ProjectRoot,'/models/sphere/umbrella/');
alpha = 1/2;
ShowPlot = 0;
...
MCError = zeros(length(MCdiv), 2, length(MCBDF));
MCErrorAll = cell(length(MCdiv), length(MCBDF));


for MCb = 1 : length(MCBDF)
    
    for MCd = 1 : length(MCdiv)
        
        clearvars -except MCb MCd MCdiv MCBDF MCErrorAll MCError ProjectRoot BDF ExactSignal FileLocation FileLocationMesh alpha ShowPlot 
...
%             [VerticesOut, FacesOut] = clearMeshDuplicates(Location, Faces );
        ...
        % % % % % % % % % %
        tau = (Sphere.Resolution/2)^2/4;
        MaxTau = 1 / Sphere.Resolution;
        NumSteps = round(MaxTau);
        % % % % % % % % % %
        
        FileNameNeigh = strcat(FileLocation,'umbrella/Icosphere_div',num2str(NumberDivisions),'_Neighbors.mat');
        
        if ~exist(FileNameNeigh, 'file')
            [Neighbors, NeighborFaces, Sphere] = findAdjacentNeighbors(Sphere);
            save(FileNameNeigh, 'Neighbors','-v7.3');
        else
           load(FileNameNeigh, 'Neighbors'); 
        end
        ...
        FileNameItL = strcat(FileLocation, 'cotangent/Icosphere_div',num2str(NumberDivisions),...
            '_ItL','_BDF',num2str(BDF),'.mat');
        
        
        if ~exist(FileNameItL, 'file')    
            ItL = makeCotangentLaplaceBeltrami( fullfile(FileLocation, FileNameSphere), BDF, tau, alpha);
...
%         Signal = performBDFDiffusion_cpp(ItL, Sphere.Signal, NumSteps);
        ...
        MCError(MCd, 1:2, MCb) = [NumSteps, AbsErr(NumSteps - 1)];
        MCErrorAll{MCd, MCb} = [(1:NumSteps)', AbsErr];
...
save(strcat(FileLocation,'cotangent/MCErrorAll.mat'), 'MCErrorAll', '-v7.3')
