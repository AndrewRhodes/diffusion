...
SamplePercVec.armadillo = [0.6,0.55,0.5,0.45,0.4];
SamplePercVec.buddha = [0.05,0.045,0.04,0.035,0.03];
SamplePercVec.bunny = [1,0.95,0.9,0.85,0.8];
SamplePercVec.dragon = [0.06, 0.055, 0.05, 0.045, 0.04];
SamplePercVec.itokawa = [0.45, 0.4, 0.35, 0.3, 0.25];
...
for k = 1 : modelDataLength
    ...
    FileLocation = strcat(ProjectRoot,'/main/DE/keypointdata/',modelData{k},'/',NoiseType,'Noise/ShortRun/');
    ...
    for kk = 1 : NoiseVecLength
        ...
        [Error{k,kk}, NoMatch{k,kk}] = findKeypointErrorNew(PointCloud, FileLocation, NoiseFileLocation, NumIter, UseNMS, t_scale, level_min, t_range);
